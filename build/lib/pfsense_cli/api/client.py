    ...
    async def authenticate(self) -> bool:
        ...
        try:
            ...
            auth_endpoints = [
                ...
                '/api/v1/access_token',  # Legacy v1 format
            ...
            # Try different endpoint and authentication method combinations
            for endpoint in auth_endpoints:
                auth_url = urljoin(self.base_url, endpoint)
                logger.debug(f"Trying authentication endpoint: {endpoint}")
                
                # For JWT endpoint, try Basic Auth first (most common)
                if endpoint == '/api/v2/auth/jwt':
                    try:
                        logger.debug(f"Trying Basic Auth with JWT endpoint: {endpoint}")
                        ...
                        if response.status_code == 200:
                            ...
                            logger.debug(f"Basic Auth failed with status {response.status_code}, trying JSON formats...")
                            
                    except requests.exceptions.RequestException as e:
                        logger.debug(f"Basic Auth request failed for {endpoint}: {e}")
                
                # Try different JSON authentication data formats
                auth_formats = [
                    {'username': self.username, 'password': self.password},       # Standard JWT format
                ...
                for auth_data in auth_formats:
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            logger.debug(f"Auth format failed with {response.status_code}, trying next format...")
                            continue
            ...
            if response is None:
                raise AuthenticationError("No valid API authentication endpoint found. Check if REST API is properly installed and enabled.")
            ...
            if response.status_code == 200:
                try:
                    ...
                    logger.debug(f"Authentication response: {token_data}")
                    
                    # Try different token response formats from pfSense API
                    ...
                    if token:
                        ...
                        logger.info(f"Successfully authenticated with pfSense API using endpoint: {successful_endpoint}")
                        return True
                    else:
                        # Some APIs might return success without token for session-based auth
                        logger.info(f"Authentication successful (session-based) using endpoint: {successful_endpoint}")
                        self._auth_token = "session-based"
                        self._auth_expires = time.time() + 3600
                        return True
                        
                except json.JSONDecodeError:
                    # Some APIs might return non-JSON success responses
                    logger.info(f"Authentication successful (non-JSON response) using endpoint: {successful_endpoint}")
                    self._auth_token = "session-based"
                    self._auth_expires = time.time() + 3600
                    return True
                ...
                raise AuthenticationError("API authentication endpoint not found. Verify REST API is installed and enabled.")
            else:
